...
const switchTimeZone = () => {
  ...
  if (selectedTask.value) {
    const localCreatedOn = new Date(
      selectedTask.value.createdOn
    ).toLocaleString('en-GB', {
      timeZone: localTimeZone.value,
    })
    const localUpdatedOn = new Date(
      selectedTask.value.updatedOn
    ).toLocaleString('en-GB', {
      timeZone: localTimeZone.value,
    })
...
<template>
  ...
  >
    <div class="bg-white w-full rounded-lg">
      <div v-if="foundTask">
        <form
          class="grid gap-[2vh] rounded-md border-none p-[2vh]"
