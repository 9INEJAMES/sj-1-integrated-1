...
<template>
    <div class="py-[20vh] px-[56vh] fixed inset-0 flex justify-center bg-black bg-opacity-50 w-full">
        <div class=" h-fit w-full rounded-lg" :class="themeStore.getTheme()">
            <div class="itbkk-modal-status grid gap-[2vh] rounded-md border-none p-[2vh]">
                ...
                <div class="text-sm sm:text-lg px-[2vh]">
                    <p class="font-medium">Users can limit the task in a status by setting maximum tasks in each status
                        (except "No Status" and "Done" statuses).</p>
                    <br />
                    <div v-if="statusReachedLimit.length > 0"
                        class="p-[2vh] rounded-lg bg-amber-200 border border-yellow-900 pb-[3vh]">
                        <p class=" text-sm">These statuses have reached the task limit. No additional tasks can be added to these statuses:
                        </p>
                        <details class="dropdown w-[20vh] h-[3vh]  ">
                            ...
                            <ul class="p-2 shadow menu dropdown-content z-[1] bg-base-100 rounded-box w-60">
                                <li v-for="status in statusReachedLimit" :key ="status.id">{{ status.name }} has reached the limit at  {{ status.noOfTasks }}</li>
                            </ul>
                        </details>
                    </div>
                    <div class="flex items-center gap-2 mt-[1vh]">
                        <label class="flex cursor-pointer gap-2">
                            <input type="checkbox" class="toggle theme-controller" @change="checkStatusLimit"
                                v-model="newLimit.limit" />
                        </label>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="number" v-model="newLimit.limitMaximumTask" min="1" :disabled="!newLimit.limit"
                ...
                <div class="flex gap-[2vh] justify-end py-[2vh]">
                    <button @click="submitSetting(true)" :disabled="!isChanged || newLimit.limitMaximumTask <= 0"
                        class="itbkk-button-save btn btn-success btn-xs sm:btn-md text-white rounded-md p-2">
                        Save
                    </button>
                    <button @click="submitSetting(false)"
                        class="itbkk-button-cancel btn btn-error btn-xs sm:btn-md text-white rounded-md p-2">Cancel</button>
                </div>
            </div>
        </div>
    </div>
