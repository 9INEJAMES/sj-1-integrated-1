...
const handleFileUpload = (e) => {
    ...
    for (const file of files.value) {
        if (file.size > 20 * 1024 * 1024) {
            ...
            files.value = Array.from(tempFiles).filter((f) => f !== file)
        }
    }
    if (errorFiles.value.length > 0) {
        errMsgArr.value.push({
            msg1: `List of files that exceed the maximum size of 20MB: `,
            msg2: `${errorFiles.value.map((file) => file.name).join(', ')}`,
        })
        errorFiles.value = []
    }

    if (tempFiles.length + newTask.value.attachments.length > maxFiles.value) {
        const left = maxFiles.value - newTask.value.attachments.length
        errorFiles.value = Array.from(tempFiles).slice(left)
        files.value = [...new Set([...files.value, ...Array.from(tempFiles).slice(0, left)])]
        console.log(files.value)
        // isChanged.value = false
        errMsgArr.value.push({
            msg1: `Each task can have at most ${maxFiles.value} files. The following files are not added: `,
            msg2: `${errorFiles.value.map((file) => file.name).join(', ')}`,
        })
        errorFiles.value = []
    }

    if (Array.from(tempFiles).some((file) => newTask.value.attachments.some((f) => f.location.split('/').pop() == file.name))) {
        errMsgArr.value.push({
            msg1: `File with the same filename cannot be added or updated to the attachements. Please delete the attachment and add again to update the file.`,
            msg2: '',
        })
        isChanged.value = false
    } else if (files) {
    ...
    if (errMsgArr.value.length > 0) {
        toastStore.changeToastArr('warn', 'Warning', errMsgArr.value)
        console.log(errMsgArr.value)
        errMsgArr.value = []
    }
}
                    ...
                    <div v-if="route.name !== 'taskAdd'" class="col-span-3">
                        <div class="flex flex-col">
                            <div v-if="$route.name == 'taskEdit'">
                                <p :class="themeStore.getTextHeaderTheme()" class="pt-[2vh]">Uploading attachment{{ files.length > 0 ? 's' : '' }}</p>
                                <div>
                            ...
                            <div>
                                ...
                                <div class="overflow-y-scroll h-fit">
                                    <div v-for="(attachments, attachmentId) in newTask.attachments" :key="attachmentId" class="overflow-y-auto">
                                        <div class="flex gap-2 items-center hover:text-pink-500 hover:underline justify-between">
                                            <p @click="attachmentApi.downloadFile(route.params.bid, newTask.id, attachments.attachmentId, attachments.location)">
                                                {{ (fileName = attachments.location.split('/').pop()) }}
                                            </p>
                                            <div class="flex gap-3">
                                                <img
                                                    v-if="fileName.endsWith('.png') || fileName.endsWith('.jpg') || fileName.endsWith('.jpg')"
                                                    :src="fileUrls[fileName]"
                                                    alt="Attachment preview"
                                                    class="w-[20px] h-[20px]"
                                                />
                                                <img v-else src="/file.png" alt="file" class="w-[15px] h-[15px]" />
                                                <img v-if="route.name !== 'taskDetails'" src="/delete.png" alt="delete" class="w-[15px] h-[15px]" @click="removeAttachmentFromTask(attachments)" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
