...
<template>
    <div class="py-[30vh] px-[10vh] fixed inset-0 flex justify-center items-center bg-black bg-opacity-35 w-full z-40">
        <div class="flex-col border rounded-md p-[2vh] w-1/4 h-fit" :class="themeStore.getTheme()">
            <p class="font-bold text-[4vh] py-[2vh]" :class="themeStore.getTextHeaderTheme()">
                Delete a {{ mode == 'task' ? 'Task' : (mode == 'status' ? 'Status' : 'Board') }}
            </p>
            <hr />
            <p v-if="mode == 'task' && !isInUsed" class="itbkk-message py-[3vh]">
                Do you want to delete the Task number {{ number }} "{{ object.title }}"
            </p>
            <p v-else-if="mode == 'status' && (object.name == 'No Status' || object.name == 'Done')"
                class="itbkk-message py-[3vh]">
                You can't delete default status
            </p>
            <p v-else-if="mode == 'board'" class="itbkk-message py-[3vh]">
                Do you want to delete the Board name "{{ object.name }}"?
            </p>
            <div v-else-if="mode == 'status'" class="pt-[3vh]">
                <p class="itbkk-message">
                    There {{ object.tasks == 1 ? 'is' : 'are' }} {{ object.tasks }} {{ object.tasks == 1 ? 'task' : 'tasks'
                    }} associated with the {{ object.name }} status
                </p>
                <div class="flex">
            ...
            <div class="flex gap-[2vh] justify-end py-[2vh]">
                ...
                <button @click="submitDelete" class="itbkk-button-confirm btn btn-success text-white rounded-md p-2"
                    :class="(mode == 'status' && isInUsed && !isSelectNewStatus) || (mode == 'status' && object.name == 'No Status') ? 'disabled' : ''"
                    :disabled="(mode == 'status' && isInUsed && !isSelectNewStatus) || (mode == 'status' && object.name == 'No Status')">
                    {{ (mode == 'status' && isInUsed && (object.name == 'No Status' || object.name == 'Done')) ? 'Transfer'
                        : 'Confirm' }}
                </button>
            </div>
        </div>
