...
const handleSubmit = async () => {
    ...
    if (props.action === 'add') {
        const collab = await collabApi.addCollaborator(newCollaborator)
        console.log(collab)
        if (collab?.success) collabStore.addCollaborator(collab)
        else collabStore.fetchCollaborator(route.params.bid)
    }
            ...
            <div class="mb-5">
                <div class="grid grid-cols-6 gap-2" v-if="props.action === 'add'">
                    ...
                    <div class="col-span-2">
                        <label for="access_right" class="block mb-1 font-medium"> Access Right</label>
                        <select class="itbkk-status select select-bordered" id="access_right" v-model="newCollaborator.access_right">
